...
**Not available, this is because some attractions do not have data stored for that year, so average functions cannot work**
...
**Suggests that there may be outliers above the upper fence**
...
There are very few attractions for Northern Ireland and Wales, so they will not have very reliable estimates
```{r Ex_l}
visitors_with_nations %>%
  count(nation)
```
